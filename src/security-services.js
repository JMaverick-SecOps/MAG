...
function securityReviewPage() {
  const cards = TIERS.map((tier) => `<a class="service-card" href="/hire?service=${tier.id}#checkout" aria-label="Select ${tier.name} — ${tier.price}"><h2>${tier.name}</h2><b>${tier.price}</b><p>${tier.limits}</p><ul>${tier.deliverables.map((item) => `<li>${item}</li>`).join("")}</ul><span class="select">Select review →</span></a>`).join("");
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>MAG Security Evidence Lab</title><link rel="icon" href="/mag-favicon.png"><style>*{box-sizing:border-box}body{max-width:1180px;margin:5vh auto;padding:0 22px;background:#061a33;color:#eaf7ff;font:16px/1.55 system-ui}a{color:#11d8ed}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:15px}.service-card{display:flex;flex-direction:column;background:#071d35;border:1px solid #28516f;border-radius:16px;padding:20px;color:inherit;text-decoration:none}.service-card:hover,.service-card:focus-visible{border-color:#11d8ed;outline:2px solid #11d8ed;outline-offset:3px}.service-card h2{font-size:1.25rem}.service-card ul{flex:1;padding-left:20px}.select{color:#11d8ed;font-weight:750}b{color:#f6c653;font-size:1.35rem}.note{color:#9eb6c9}.notice{border-left:3px solid #f6c653;padding:14px 18px;margin:25px 0;background:#0a2744}.brand{display:flex;align-items:center;gap:12px}.brand img{width:48px;height:48px;object-fit:contain}@media(max-width:920px){.grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:600px){.grid{grid-template-columns:1fr}}</style></head><body><a class="brand" href="/"><img src="/mag-app-icon.png" alt="">MAG · Agent Guild</a><h1>Security Evidence Lab</h1><p>Choose your review. We prefill the package, deliverables and price; you supply the authorized repository and exact commit.</p><section class="grid" aria-label="Select a security service">${cards}</section><p class="notice">Scope first, payment second. Isolated review capacity must be confirmed before an invoice can be paid. No charge is taken for this intake, and no immediate report delivery is promised.</p><p class="note">Static and review-only by default. MAG never executes submitted code on the production Worker and does not accept repository credentials through this API. Findings include evidence, confidence, CWE mapping, remediation and verification steps.</p><p><a href="/hire">Browse every service →</a></p></body></html>`;
}
